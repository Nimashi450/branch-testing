...
console.log("us_location--------->"+us_location);
...
function onMessageArrived(r_message){
    ...
    for (let i = 0; i < id_location.length; i++) {
        ...
        if(topic=="slt_agro21/data/"+userid+"/"+id_location[i] && IsJsonString(r_message.payloadString))
        ...
        // You need to change this if a user has more than one farm 
...
for (let i = 0; i < id_location.length; i++) {
    mqtt.subscribe("slt_agro21/data/"+userid+"/"+id_location[i]);
    }
...
function send_message(){
    ...
    message.destinationName = 'slt_agro21/data_in/'+userid;
